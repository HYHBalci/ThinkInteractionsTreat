...
simulate_data <- function(n_samples, p_main, p_noise_main, interaction = TRUE, treatment = TRUE, seed = 123) {
  ...
  if (interaction) {
    ...
    chosen_main_main <- sample(1:nrow(main_main_pairs), non_zero_interactions$main_main)
    for (i in chosen_main_main) {
      pair <- main_main_pairs[i, ]
      interaction_effects[pair[1], pair[2]] <- runif(1, 0.2, 0.5)
    ...
    chosen_main_absent <- chosen_main_absent[sample(1:nrow(chosen_main_absent), non_zero_interactions$main_absent), ]
    ...
    chosen_absent_absent <- chosen_absent_absent[sample(1:nrow(chosen_absent_absent), non_zero_interactions$absent_absent), ]
